...
public class PhotonVisionSubsystem extends SubsystemBase {
  ...
  */
  public PhotonVisionSubsystem(String hostName, int pl) {
  ...
  public void periodic() {
    if(_nt.getEntry("hasTarget").getBoolean(false)) {
      ...
      timer.reset();
      timer.start();
      tx = (float)_nt.getEntry("targetYaw").getDouble(0);
      ty = (float)_nt.getEntry("targetPitch").getDouble(0);
    }
    updateWidgets();
  }
  ...
  public void driverMode(Boolean b) {
    dMode = b;
    if (b) _nt.getEntry("driverMode").setNumber(1);
    else _nt.getEntry("driverMode").setNumber(0);
  }
  ...
  public void setPipeLine(Integer p) {
    _nt.getEntry("pipelineIndex").setNumber(p);
  }
